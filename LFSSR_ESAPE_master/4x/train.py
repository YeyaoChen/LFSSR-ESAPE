    ...
    for idx_epoch in range(start_epoch, args.epoch):
        ...
        if idx_epoch % step == 0 or idx_epoch > args.epoch - step:
            with torch.no_grad():
                ...
                logger.log_string('The %dth val, psnr/ssim is %.3f/%.4f' % (idx_epoch + 1, psnr_epoch_test, ssim_epoch_test))

                vlog2.log_step({'val_psnr': psnr_epoch_test, 'val_ssim': ssim_epoch_test})
                time.sleep(0.05)
...
def train(train_loader, device, net, criterion, optimizer, logger):
    ...
    for idx_iter, (data, label, ref_y, hr_down2) in tqdm(enumerate(train_loader), total=len(train_loader), ncols=70):
        # '''Memory Usage'''
        ...
        out_lf, out_spa_lbp, out_ang_lbp = net(data, ref)
        ground_truth = [label, hr_down2]
        loss_total = criterion(out_lf, out_spa_lbp, out_ang_lbp, ground_truth)
        ...
        psnr, ssim = cal_metrics(args, label, out_lf[0])
...
def test(test_loader, device, net, epoch_dir=None, CenterView_dir=None, mat_file=None, aligned_image_dir=None):
    ...
    LF_iter_test = []
    psnr_iter_test, ssim_iter_test, psnr_iter_test_allviews, ssim_iter_test_allviews = [], [], [], []
    ...
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, ref_y, Sr_SAI_cbcr, LF_name) in tqdm(enumerate(test_loader), total=len(test_loader), ncols=70):
        ...
        for i in range(0, patch_num, args.minibatch_for_test):
            ...
            with torch.no_grad():
                ...
                output_ = rearrange(output[0],'1 1 (an1 h) (an2 w)->1 1 (h an1) (w an2)', an1=5, an2=5, h=patch_ref, w=patch_ref)
