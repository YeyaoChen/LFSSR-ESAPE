...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        scale_x2, disp_num_x2 = 4, 15
        scale_x4, disp_num_x4 = 2, 25
        scale_x8, disp_num_x8 = 1, 35
        self.plf_x2 = PLFConstruct(self.channels, scale_x2, self.angRes, 1.0, disp_num_x2)   # [(b ah aw),(s n c),h,w]
        self.plf_x4 = PLFConstruct(self.channels, scale_x4, self.angRes, 2.0, disp_num_x4)
        self.plf_x8 = PLFConstruct(self.channels, scale_x8, self.angRes, 4.0, disp_num_x8)
    ...
    def forward(self, in_lr, in_ref):
        ...
        #############################################  Stage3: 8x  #############################################
        lr_img_x8 = functional.interpolate(rec_x4, scale_factor=2, mode="bicubic", align_corners=False)
        spa_lbp_x8, ang_lbp_x8, spa_lbp_img_x8, ang_lbp_img_x8 = self.lbp_enhancer(lr_img_x8)

        fea_x8 = self.con_align(lr_img_x8, self.plf_x8(in_ref))             # [(b,ah,aw),c,8h,8w]
        fea_x8 = self.fea_enhance(fea_x8, spa_lbp_x8, ang_lbp_x8)
        res_x8 = self.lf_rec(fea_x8)
        rec_x8 = res_x8 + lr_img_x8

        rec_lf = [to_sai_array(rec_x8, self.angRes), to_sai_array(rec_x4, self.angRes), to_sai_array(rec_x2, self.angRes)]
        rec_spa_lbp = [to_sai_array(spa_lbp_img_x8, self.angRes), to_sai_array(spa_lbp_img_x4, self.angRes), to_sai_array(spa_lbp_img_x2, self.angRes)]
        rec_ang_lbp = [ang_lbp_img_x8, ang_lbp_img_x4, ang_lbp_img_x2]
...
class get_loss(nn.Module):
    ...
    def forward(self, rec_x, spa_lbp, ang_lbp, gt):
        rec_x8, rec_x4, rec_x2 = rec_x[0], rec_x[1], rec_x[2]
        spa_lbp_x8, spa_lbp_x4, spa_lbp_x2 = spa_lbp[0], spa_lbp[1], spa_lbp[2]
        ang_lbp_x8, ang_lbp_x4, ang_lbp_x2 = ang_lbp[0], ang_lbp[1], ang_lbp[2]
        gt_x8, gt_x4, gt_x2 = gt[0], gt[1], gt[2]

        in_ah = self.angRes
        in_aw = self.angRes

        gt_spa_x8 = rearrange(gt_x8, 'b c (ah h) (aw w) -> (b ah aw) c h w', ah=in_ah, aw=in_aw)
        ...
        gt_spa_x8 = to_sai_array(self.spa_lbp(gt_spa_x8), self.angRes)
        gt_spa_x4 = to_sai_array(self.spa_lbp(gt_spa_x4), self.angRes)
        gt_spa_x2 = to_sai_array(self.spa_lbp(gt_spa_x2), self.angRes)

        gt_ang_x8 = self.ang_lbp(rearrange(gt_x8, 'b c (ah h) (aw w) -> b c (h ah) (w aw)', ah=self.angRes, aw=self.angRes))
        ...
        rec_loss = self.l1(rec_x8, gt_x8) + self.l1(rec_x4, gt_x4) + self.l1(rec_x2, gt_x2)
        spa_lbp_loss = self.l1(spa_lbp_x8, gt_spa_x8) + self.l1(spa_lbp_x4, gt_spa_x4) + self.l1(spa_lbp_x2, gt_spa_x2)
        ang_lbp_loss = self.l1(ang_lbp_x8, gt_ang_x8) + self.l1(ang_lbp_x4, gt_ang_x4) + self.l1(ang_lbp_x2, gt_ang_x2)
